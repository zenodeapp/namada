            ...
            res => {
                ...
                if is_accepted {
                    ...
                    extended_tx_result.is_ibc_shielding =
                        tx::action::is_ibc_shielding_transfer(state)
                            .map_err(Error::StateError)?;
    ...
    CA: 'static + WasmCacheAccess + Sync,
{
    ...
    let mut gas_meter = TxGasMeter::new(
        Gas::from_whole_units(max_gas_limit, gas_scale).ok_or_else(|| {
    ...
    let valid_batched_tx_result = {
        match apply_wasm_tx(
            ...
            Ok(result) => {
                ...
                let masp_ref = tx::action::get_masp_section_ref(*state)
                    .map_err(Error::StateError)?;
